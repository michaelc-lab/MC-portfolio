...
export default function StockTable({ rows, isWatchlist = false, onAnalyze }) {
  ...
  const sectorCounts = useMemo(() => {
    const c = {}
    rows.forEach(r => { if (r.sector) c[r.sector] = (c[r.sector] || 0) + 1 })
    return c
  }, [rows])

  const filtered = useMemo(() => filterRows(rows, filter, isWatchlist), [rows, filter, isWatchlist])
      ...
      <div className="panel overflow-auto max-h-[600px]">
        <table className="data-table">
          <thead>
            <tr>
              {cols.map(c => (
                <th key={c.key} onClick={() => handleSort(c.key)} className={c.num ? 'text-right' : ''}>
