...
function MarketIndicator() {
  ...
  const cfg = {
    open:     { color: 'bg-terminal-green', text: 'Market Open',    border: 'border-terminal-green/30', bg: 'bg-terminal-green/10', txt: 'text-terminal-green' },
    extended: { color: 'bg-terminal-amber', text: 'Extended Hours', border: 'border-terminal-amber/30', bg: 'bg-terminal-amber/10', txt: 'text-terminal-amber' },
    closed:   { color: 'bg-terminal-red',   text: 'Market Closed',  border: 'border-terminal-red/30',   bg: 'bg-terminal-red/10',   txt: 'text-terminal-red'   },
  }
  const c = cfg[status]
  return (
    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded border font-mono text-[11px] ${c.border} ${c.bg} ${c.txt}`}>
      <span className={`w-2 h-2 rounded-full ${c.color} animate-pulse-slow`} />
...
function StripItem({ icon, label, value, change, iconBg }) {
  ...
  return (
    <div className="flex items-center gap-2.5 px-3 py-1.5 rounded border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
      {icon && <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold text-navy-950 flex-shrink-0 ${iconBg}`}>{icon}</span>}
    ...
    <header className="border-b border-electric-500/10 bg-navy-900/80 backdrop-blur-xl sticky top-0 z-50">
      ...
      <div className="px-6 py-2 flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <StripItem label="VIX"  value={indexes.VIX  ? indexes.VIX.toFixed(2) : '—'} change={indexes.VIX_change}  iconBg="bg-electric-400" />
        </div>
      </div>
    </header>
  )
