...
function PriceChart({ chart, earnings }) {
...
function AnalyzeView({ myTickers, portfolio, watchlist }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="panel p-4 border-red-500/20 bg-red-500/5 text-[12px] font-mono text-red-400">{error}</div>
      )}

      {loading && (
        <div className="panel p-12 text-center font-mono text-[11px] uppercase tracking-[0.2em] text-electric-400 animate-pulse">
        ...
        <div className="space-y-4 animate-slide-up">
          ...
          <div className="panel-bright p-5">
            <div className="flex items-center gap-4 flex-wrap">
              {data.profile?.logo && (
                <img src={data.profile.logo} alt="" className="w-14 h-14 rounded-xl bg-navy-800 p-1.5 border border-white/10 object-contain" />
              )}
            ...
            <div className="col-span-2 panel p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display font-semibold text-[13px] text-slate-200">1-Year Price</span>
                </div>
                <span className="font-mono text-[10px] text-slate-600 uppercase tracking-wider">
                  {data.earnings?.length ? '◆ = earnings' : 'Daily closes'}
                </span>
              </div>
              <div className="h-64"><PriceChart chart={data.chart} earnings={data.earnings} /></div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="panel overflow-hidden">
              <div className="px-4 py-3 border-b border-white/5 font-display font-semibold text-[13px] text-slate-200">Recent Earnings</div>
            ...
            <div className="panel overflow-hidden">
              ...
              <div className="overflow-auto max-h-64 divide-y divide-white/[0.04]">
                {!data.news?.length
                  ? <div className="p-6 text-center text-slate-600 font-mono text-[11px]">No news</div>
                  : data.news.map((n, i) => (
                    <div key={i} className="px-4 py-3 hover:bg-white/[0.02] transition-colors">
                      <a href={n.url} target="_blank" rel="noopener noreferrer" className="text-[12px] text-slate-300 hover:text-electric-300 font-medium leading-snug block mb-1 transition-colors">
                        {n.headline}
                      </a>
...
function CompareView({ portfolio, watchlist }) {
  ...
  const run = async () => {
    ...
    setLoading(true); setError(null); setData(null)
    try {
      const d = await fetchCompareData(a, b)
      setData(d)
    } catch (e) { setError(e.message) }
    finally { setLoading(false) }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 items-center flex-wrap">
        {[['A', tickerA, setTickerA, allStocks], ['B', tickerB, setTickerB, allStocks]].map(([label, val, set, stocks]) => (
          <div key={label} className="flex gap-2 flex-1 min-w-64">
            ...
            <select
              value={stocks.some(r => r.ticker === val) ? val : ''}
            ...
            >
              ...
              {stocks.map(r => <option key={r.ticker} value={r.ticker}>{r.ticker} — {r.company}</option>)}
            </select>
            <input
              value={val}
            ...
            <tbody>
              {METRICS.map(m => {
                ...
                if (m.higher !== null && m.a != null && m.b != null) {
                  if ((m.higher && m.a > m.b) || (!m.higher && m.a < m.b)) {
                    clsA = 'font-mono text-[12px] font-bold positive'
                    clsB = 'font-mono text-[12px] text-slate-500'
                  } else if ((m.higher && m.b > m.a) || (!m.higher && m.b < m.a)) {
                    clsB = 'font-mono text-[12px] font-bold positive'
                    clsA = 'font-mono text-[12px] text-slate-500'
                  }
                }
                return (
                  <tr key={m.label}>
